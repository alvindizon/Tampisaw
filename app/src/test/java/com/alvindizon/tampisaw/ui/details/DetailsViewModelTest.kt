...
class DetailsViewModelTest {
    ...
    fun `observe correct states when getPhotos is successful`() {
        ...
        assertEquals(observedValues[0], DetailsUIState.LOADING)
        assertEquals(
            observedValues[1],
            DetailsUIState.SUCCESS(getPhotoResponse.toPhotoDetails())
        )
    }
    ...
    fun `observe correct states when getPhotos is not successful`() {
        ...
        val observedValues = uiStatus.observedValues
        assertEquals(observedValues[0], DetailsUIState.LOADING)
        assertEquals(observedValues[1], DetailsUIState.ERROR("erreur"))
    }
